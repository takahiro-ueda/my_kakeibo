  ...
  def save
    ...
    FixedcostValue.transaction {
      self.fixedcost_values.select.each { |fixedcost_value|
        a1 = FixedcostValue.new(:fixedcost_id => fixedcost_value.fixedcost_id, :year_month => fixedcost_value.year_month,
            :value => fixedcost_value.value, :description => fixedcost_value.description)
        a1.save!
      }
    }
    true
